...
function getErrorMessage(error) {
  ...
  if (message.indexOf('WECHAT_OCR_BASE64_UNSUPPORTED') >= 0) return '云存储上传失败，且腾讯云 OCR 尚未配置，请先检查云存储权限或配置腾讯云 OCR。'
  if (message.indexOf('48001') >= 0 || /permission|authorize|access denied/i.test(message)) return '微信 OCR 没有权限，请配置腾讯云 OCR 环境变量后重试。'
  if (/timeout|timed out/i.test(message)) return 'OCR 请求超时，请稍后重试。'
...
function hasTencentConfig() {
  return Boolean(process.env.TX_OCR_SECRET_ID || process.env.TENCENTCLOUD_SECRET_ID) &&
    Boolean(process.env.TX_OCR_SECRET_KEY || process.env.TENCENTCLOUD_SECRET_KEY)
}
...
async function recognizeImage(fileID, imgUrl, imageBase64) {
  const requestData = {}
  if (imageBase64) {
    const normalized = String(imageBase64).replace(/^data:image\/[a-zA-Z0-9.+-]+;base64,/, '')
    const fileContent = Buffer.from(normalized, 'base64')
...
async function recognizeWechatImage(fileID, imgUrl) {
  let imageUrl = imgUrl || ''
  if (!imageUrl && fileID) {
    const tempFileRes = await cloud.getTempFileURL({
      fileList: [fileID]
    ...
    const file = tempFileRes.fileList && tempFileRes.fileList[0]
    imageUrl = file && file.tempFileURL ? file.tempFileURL : ''
  }
...
exports.main = async function (event) {
  try {
    ...
    if (!imgUrl && !event.fileID && !event.imageBase64) {
      return {
        text: '',
    ...
    if (hasTencentConfig()) {
      return await recognizeImage(event.fileID || '', imgUrl, event.imageBase64 || '')
    }

    if (event.imageBase64 && !event.fileID && !imgUrl) {
      throw new Error('WECHAT_OCR_BASE64_UNSUPPORTED')
    }

    return await recognizeWechatImage(event.fileID || '', imgUrl)
  } catch (error) {
    return {
      text: '',
      error: 'ocr_failed',
